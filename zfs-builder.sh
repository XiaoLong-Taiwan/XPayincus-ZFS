...
#
# 产出: ./output/xpayincus-zfs-<ZFS版本>-linux-<架构>-<内核版本>.tar.gz
# ============================================================================
set -euo pipefail
...
show_kernel_list() {
    ...
    for i in "${!KERNEL_LIST[@]}"; do
        ...
        # 检查是否已有预编译包
        local has_pkg=""
        if compgen -G "${OUTPUT_DIR}/xpayincus-zfs-*-linux-${ARCH}-${kver}.tar.gz" >/dev/null; then
...
show_packages() {
    ...
    for f in "${OUTPUT_DIR}"/xpayincus-zfs-linux-*.tar.gz; do
        ...
        # 提取内核版本
        local kver="${fname#xpayincus-zfs-}"
        kver="${kver#*-linux-${ARCH}-}"
        kver="${kver%.tar.gz}"
...
build_for_kernel() {
    ...
    # 检查是否已有包
    if compgen -G "${OUTPUT_DIR}/xpayincus-zfs-*-linux-${ARCH}-${kernel_ver}.tar.gz" >/dev/null; then
    ...
    # 打包
    local package_name="xpayincus-zfs-${zfs_ver}-linux-${ARCH}-${kernel_ver}"
